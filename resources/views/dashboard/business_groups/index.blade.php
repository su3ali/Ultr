                    ...
                    <nav class="breadcrumb-one" aria-label="breadcrumb">
                        <ol class="breadcrumb mb-0 py-2">
                            ...
                            <li class="breadcrumb-item active" aria-current="page"> {{
                                __('dash.business_technician_groups') }}</li>
                        </ol>
                    </nav>
...
@include('dashboard.business_groups.create')
@endsection
                ...
                <table id="html5-extension" class="table table-hover non-hover" style="width:100%">
                    <thead>
                        <tr>
                            <th>#</th>
                            <th>{{__('dash.name')}}</th>
                            <th>{{ __('dash.group_supervisor') }}</th>
...
@include('dashboard.business_groups.edit')
@endsection
@push('script')
<script>
...
<script type="text/javascript">
    $(document).ready(function () {
            $('#html5-extension').DataTable({
                ...
                processing: true,
                 serverSide: false,
                ajax: '{{ route('dashboard.businessGroup.index') }}',
                columns: [
                    {data: 'id', name: 'id'},
    ...
    // Load cities after country change (with delay after AJAX completes)
    $.ajax({
        url: '{{ route('dashboard.core.address.getCity') }}',
        data: {country_id: country_id},
        success: function (response) {
            ...
            // Now set the selected city_id
            $('#city_id').val(city_id).trigger('change');
        }
    });

    // Form action update
    let action = "{{ route('dashboard.businessGroup.update', 'id') }}";
    ...
    $('#edit_grouptech_form').attr('action', action);
        ...
        $("body").on('change', '#customSwitch4', function () {
            ...
            $.ajax({
                url: '{{route('dashboard.businessGroup.change_status')}}',
                type: 'get',
