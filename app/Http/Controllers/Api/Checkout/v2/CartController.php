    ...
    public function changeOrderSchedule(Request $request)
    {
        ...
        try {
            ...
            if ($order->is_refunded === 1) {
                return self::apiResponse(400, __('dash.order_is_refunded'));
            }
            ...
            $oldStartTime = $visit->getOriginal('start_time');
            $oldEndTime   = $visit->getOriginal('end_time');
            $oldGroupId   = $visit->getOriginal('assign_to_id');
            ...
            // Log changes
            ActivityLogger::log(
                actionType: 'visit_rescheduled',
                model: $visit,
                description: 'تم إعادة جدولة الزيارة',
                userId: auth()->id(),
                changes: [
                    'start_time' => ['from' => $oldStartTime, 'to' => $visit->start_time],
                    'end_time'   => ['from' => $oldEndTime, 'to' => $visit->end_time],
                    'group_id'   => ['from' => $oldGroupId, 'to' => $visit->assign_to_id],
                ],
                meta: [
                    'booking_id'   => $booking->id,
                    'order_id'     => $order->id,
                    'visit_id'     => $visit->id,
                    'region_id'    => optional($address)->region_id,
